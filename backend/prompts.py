...
3. Centralização e Mapeamento de Gráficos e Simuladores Interativos: Analise as recomendações de simulador (`simuladores_interativos_recomendados`) trazidas por cada página. Mapeie no campo 'simuladores_da_aula' os simuladores e gráficos interativos que façam real sentido didático com os subtópicos da aula (por exemplo: em uma aula de histogramas, é importante ter um histograma interativo; em regressão linear, o ajuste da reta e dispersão de resíduos). Permita quantos simuladores forem necessários para a rica compreensão do conteúdo, sem limite máximo fixado, mas também NÃO é obrigatório ter nenhum se a aula for puramente teórica/abstrata sem necessidade de visualização gráfica. Se um subtópico não tiver relevância visual direta, não atribua simulador a ele. É PERMITIDO atribuir mais de um simulador para a mesma página (repetindo o 'indice_pagina') se o subtópico demandar múltiplas visualizações distintas.
